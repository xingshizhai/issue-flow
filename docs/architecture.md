...
- Fake Provider 的 `data_file` 只能是配置目录内的直接文件名，不允许绝对路径、
  ...
  防止本地演练读写项目外数据。
- 配置文件最大 1 MiB，必须是普通文件且不得为符号链接；读取时核对打开前后的
  文件身份。`init` 使用排他创建，不采用先检查再覆盖的方式，并拒绝悬空链接，
  防止配置路径竞态和项目外写入。
- Gitee REST Token 配置只接受大写 `GITEE_*TOKEN*` 环境变量名，防止不可信仓库
  把任意系统、云平台或其他 Provider 凭据作为 Gitee access token 外发。
